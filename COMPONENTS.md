...
`section-dark` · `section-header` · `eyebrow` · `rule-accent` · `split` ·
`split-even` · `split-reverse` · `prose` · `prose-meta`

**Components:** `btn` · `btn-primary` · `btn-secondary` · `btn-group` ·
`card-grid` · `card-grid-wide` · `card` · `card-link` · `stat-card` ·
`stat-card-mark` · `process` · `quote` · `pill-list` · `cta-band`

**Calculators:** `calc` · `calc-result` · `calc-result-value` ·
`calc-result-detail` · `calc-note`
...
Never write a hex code. Use these:

`--color-brand` · `--color-brand-dark` · `--color-brand-tint` ·
`--color-sage` · `--color-sage-light` · `--color-gold` ·
`--color-gold-light` · `--color-stone` · `--color-mist` · `--color-text` ·
`--color-text-muted` · `--color-bg` · `--color-bg-alt` · `--color-border` ·
`--color-border-strong`

**Contrast warning.** On white, only `--color-brand` and `--color-text` are
safe for text. `--color-sage` and `--color-gold` fail the minimum contrast
ratio for body text — they are for borders, rules and decoration only.
Gold is readable on dark green, and only there.
...
| `section section-alt` | Warm cream background |
| `section section-tint` | Pale brand-coloured background |
| `section section-dark` | Dark green background, light text |

Alternate these down a page so adjacent sections separate visually. Never
two the same in a row — and remember the footer is cream, so don't end a
page on `section-alt`.

**`section-dark` is an accent, not an alternative.** One or two per page at
most, usually the hero or a mid-page break. Everything inside it — text,
buttons, cards, pill lists — recolours automatically, so you don't need to
do anything except add the class.

There's also `rule-accent`, a short gold line for under a heading:
...
A card grid where each card leads with a large mark — a number, a symbol,
a short string. Used for "Why choose us".

```html
<ul class="card-grid">

  <li class="card stat-card">
    <span class="stat-card-mark">100%</span>
    <h3>100% pass ratio</h3>
    <p>Every application we submit is prepared properly the first time.</p>
  </li>

  <li class="card stat-card">
    <span class="stat-card-mark">1:1</span>
    <h3>Personalised, one-to-one support</h3>
    <p>You get a named adviser who knows your circumstances.</p>
  </li>

</ul>
```

Both classes on the `<li>` — `card` gives the box, `stat-card` adds the
mark styling.

Keep marks to four characters or fewer. Symbols work as well as numbers:
`∞`, `✓`, `→`.
...
A numbered "here's what happens" sequence. Used on the residential
mortgage pages.
...
<ol class="process">
  ...
  <li>
    <h3>Contact your adviser</h3>
    <p>We introduce ourselves and explain how it all works.</p>
  </li>

  <li>
    <h3>Fact find</h3>
    <p>We get to know your income, outgoings and goals.</p>
  </li>

  <li>
    <h3>Decision in Principle</h3>
    <p>We confirm roughly what you can borrow.</p>
  </li>

</ol>
```

**You never type the numbers.** They're generated by CSS. Add a step,
remove one, reorder them — the numbering corrects itself. If you type "1."
into the heading you'll end up with "1 1. Contact your adviser".

Use `<ol>`, not `<ul>` — the order is the whole point, and screen readers
announce it as an ordered list.

Reflows automatically: four columns on a desktop, one on a phone. Works
with any number of steps, including the ten-step first time buyer process.

---

## Calculators

The inputs are ordinary `.form-field` blocks. These classes cover the
panel and the result readout.
...
<div class="calc">
  ...
  <div class="form-field">
    ...
    <input type="number" id="price" name="price" value="300000">
  </div>

  <div class="form-field">
    <label for="deposit">Deposit (£)</label>
    <input type="number" id="deposit" name="deposit" value="30000">
  </div>

  <div class="calc-result">
    <p class="calc-result-value" id="calc-output">£1,500 / mo</p>
    <p class="calc-result-detail" id="calc-detail">Loan amount £270,000 · LTV 90%</p>
  </div>

  <p class="calc-note">
    Estimated capital and interest repayment mortgage. Excludes fees. This
    is a guide only and is not a mortgage offer or personal recommendation.
  </p>

</div>
```

### Do not remove `.calc-note`

The calculators are a regulated financial promotion. The output has to be
clearly presented as an illustration, never as an offer or a
recommendation. The exact wording has to be signed off by Flexi Network
before launch — if you change it, tell Sen.
...
| A dark green band | Add `section-dark` |
| A short gold rule | `<hr class="rule-accent">` |
| A row of boxes | `<ul class="card-grid">` + `<li class="card">` |
| Boxes leading with a big number | Add `stat-card` to each `<li>` |
| Numbered steps in order | `<ol class="process">` |
| A calculator | `<div class="calc">` + `.form-field`s + `.calc-result` |
